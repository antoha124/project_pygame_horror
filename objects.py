...
class SG(pygame.sprite.Sprite):
    def __init__(self, pos, end_doors):
        ...
        self.rect = self.image.get_rect()
        # Координаты
        self.rect.x = x * CELL_W + CELL_W // 2 - self.rect.w // 2
        self.rect.y = y * CELL_W + CELL_W // 2 - self.rect.w // 2
        # Логика
            ...
            for ray in range(NUM_RAYS):
                ...
                for i in range(len(self.w_map)):
                    ...
                    if self.w_map[int(x // CELL_W - m) % (MAZE_S * 2 + 1), int(yv // CELL_W) % (MAZE_S * 2 + 1)] != 1:
                        obj_v = self.w_map[
                            int(x // CELL_W - m) % (MAZE_S * 2 + 1), int(yv // CELL_W) % (MAZE_S * 2 + 1)]
                        break
                    x += dx * CELL_W
                # Горизонтали
                ...
                for i in range(len(self.w_map[cp_x // CELL_W])):
                    ...
                    if self.w_map[int(xh // CELL_W) % (MAZE_S * 2 + 1), int(y // CELL_W - m) % (MAZE_S * 2 + 1)] != 1:
                        obj_h = self.w_map[
                            int(xh // CELL_W) % (MAZE_S * 2 + 1), int(y // CELL_W - m) % (MAZE_S * 2 + 1)]
                        break
                    y += dy * CELL_W
                ...
                if any((btns[BTN_F], btns[BTN_B], btns[BTN_L], btns[BTN_R])):
                    if not pygame.key.get_mods():
                        ...
                        try:
                            ...
                            print(e)
                        self.step = FPS // 4 if self.stamina.stamina >= 1 else FPS // 2
            except Exception as e:
                print(e)
        self.step -= 1
